...
class HashTableSC(object):
    def __init__(self):
        self.table_size = 512
        self.arr_list = [[] for _ in range(self.table_size)]
    ...
    def add(self, key, value):
        ...
        self.arr_list[index].append((key, value))

    def get(self, key):
        index = self.compute_hash(key)
        for pair in self.arr_list[index]:
            if pair[0] == key:
                return pair[1]
        return sys.maxsize
    ...
    def remove(self, key):
        ...
        for pair in self.arr_list[index]:
            if pair[0] == key:
                self.arr_list[index].remove(pair)
            return True
        return False

    def print(self):
        for i in range(self.table_size):
            if len(self.arr_list[i]) != 0:
                for keyvalue in self.arr_list[i]:
